...
INT UParse::Main( const TCHAR* Parms )
{
	...
	if( Parse(Parms,TEXT("project="), Project) )
	{
		...
		if( Project.InStr(FString(TEXT(".upc"))) != -1 )
		{
			...
			if( !ConfigCache->GetString(TEXT("project"), TEXT("output"), DateType, MainProjectPath ) )
			{
				output = FString(TEXT("classes"));
			}
			if( !ConfigCache->GetString(TEXT("project"), TEXT("input"), DateType, MainProjectPath ) )
			{
				input = FString(TEXT("classes/preprocessor"));
			}
		...
		{		
			...
			if(appLoadFileToString(S,*t_source))
			{
				...
				if( S.InStr( FString(TEXT("`process"))) != -1 )
				{														
...
FString UParse::ParseSource(FString Text, UBOOL& bError, UBOOL& bCrash)
{
	...
	while(Text.Len() > 0)
	{
		...
		//namespace
		if(VNameSpaceString != FString(TEXT("")) && !SkipLine && !IsDirective(tmpStr))
		{
			...
			{		
				...
				else
				{
					...
					if( ImpInfo.Group != FString(TEXT("")) )
						FString(TEXT("GROUP="))+ImpInfo.Group;
		...
		if(include_file && !inc_parse) result += InclusionString;
		// END: parses tmpStr
		if(linebreak == -1)
		{
			if(include_file && inc_parse) Text = InclusionString + Text;
			Text = FString(TEXT(""));
		}
		else
		{
