/*=============================================================================
	...
	* Distance fog supprot for UT'99 added by Raven
	* Big THX to Enigma for accessing levelinfo code and to moonangel for fixing this glowing STY_Translucent actors bug
...
static const TCHAR *g_pSection = TEXT("ROpenGLDrv.OpenGLRenderDevice");
...
static const char *g_vpComplexSurfaceDoubleTexture =
	"!!ARBvp1.0\n"
	"OPTION ARB_position_invariant;\n"
...
#define CPP_PROPERTY_LOCAL(name) name, CPP_PROPERTY(name)
	...
	SC_AddBoolConfigParam(12, TEXT("UseZTrick"), CPP_PROPERTY_LOCAL(UseZTrick), 0);
	SC_AddBoolConfigParam(11, TEXT("UseBGRATextures"), CPP_PROPERTY_LOCAL(UseBGRATextures), 1);
	SC_AddBoolConfigParam(10, TEXT("UseMultiTexture"), CPP_PROPERTY_LOCAL(UseMultiTexture), 1);
	SC_AddBoolConfigParam(9,  TEXT("UsePalette"), CPP_PROPERTY_LOCAL(UsePalette), 0);
	SC_AddBoolConfigParam(8,  TEXT("ShareLists"), CPP_PROPERTY_LOCAL(ShareLists), 0);
	SC_AddBoolConfigParam(7,  TEXT("AlwaysMipmap"), CPP_PROPERTY_LOCAL(AlwaysMipmap), 0);
	SC_AddBoolConfigParam(6,  TEXT("UsePrecache"), CPP_PROPERTY_LOCAL(UsePrecache), 0);
	SC_AddBoolConfigParam(5,  TEXT("UseTrilinear"), CPP_PROPERTY_LOCAL(UseTrilinear), 0);
	SC_AddBoolConfigParam(4,  TEXT("AutoGenerateMipmaps"), CPP_PROPERTY_LOCAL(AutoGenerateMipmaps), 0);
	...
	SC_AddBoolConfigParam(1,  TEXT("NoMaskedS3TC"), CPP_PROPERTY_LOCAL(NoMaskedS3TC), 0);
	...
	SC_AddBoolConfigParam(7,  TEXT("SinglePassDetail"), CPP_PROPERTY_LOCAL(SinglePassDetail), 0);
	...
	SC_AddBoolConfigParam(2,  TEXT("UseMultiDrawArrays"), CPP_PROPERTY_LOCAL(UseMultiDrawArrays), 0);
	SC_AddBoolConfigParam(1,  TEXT("UseCVA"), CPP_PROPERTY_LOCAL(UseCVA), 0);
	SC_AddBoolConfigParam(0,  TEXT("UseVertexProgram"), CPP_PROPERTY_LOCAL(UseVertexProgram), 0);
...
#ifdef UTGLR_DX_BUILD
	SC_AddBoolConfigParam(4,  TEXT("SceneNodeHack"), CPP_PROPERTY_LOCAL(SceneNodeHack), 0);
#endif
	SC_AddBoolConfigParam(3,  TEXT("SmoothMaskedTextures"), CPP_PROPERTY_LOCAL(SmoothMaskedTextures), 0);
	SC_AddBoolConfigParam(2,  TEXT("MaskedTextureHack"), CPP_PROPERTY_LOCAL(MaskedTextureHack), 0);
	...
	SC_AddIntConfigParam(TEXT("AAFilterHint"), CPP_PROPERTY_LOCAL(AAFilterHint), 0);
	SC_AddBoolConfigParam(1,  TEXT("ZRangeHack"), CPP_PROPERTY_LOCAL(ZRangeHack), 0);
	SC_AddBoolConfigParam(0,  TEXT("UseTruForm"), CPP_PROPERTY_LOCAL(UseTruForm), 0);
	SC_AddIntConfigParam(TEXT("TruFormTessellation"), CPP_PROPERTY_LOCAL(TruFormTessellation), 3);
	SC_AddIntConfigParam(TEXT("TruFormMinVertices"), CPP_PROPERTY_LOCAL(TruFormMinVertices), 0);
//distance fog stuff	
	SC_AddFloatConfigParam(TEXT("FogColorR"), CPP_PROPERTY_LOCAL(FogColorR), 128);
	SC_AddFloatConfigParam(TEXT("FogColorG"), CPP_PROPERTY_LOCAL(FogColorG), 128);
	SC_AddFloatConfigParam(TEXT("FogColorB"), CPP_PROPERTY_LOCAL(FogColorB), 128);
	SC_AddFloatConfigParam(TEXT("FogDistance"), CPP_PROPERTY_LOCAL(FogDistance), 1024);
	SC_AddBoolConfigParam(1,  TEXT("ForceDistanceFog"), CPP_PROPERTY_LOCAL(ForceDistanceFog), 0);
	SC_AddBoolConfigParam(1,  TEXT("SpawnIfNoDistanceFog"), CPP_PROPERTY_LOCAL(SpawnIfNoDistanceFog), 0);
...
void UOpenGLRenderDevice::BufferAdditionalClippedVerts(FTransTexture** Pts, INT NumPts) {
	...
	do {
		...
		if (UseTruForm) {
			NormalArray[BufferedVerts] = NormalArray[FirstVert];
		}
		...
		if (UseTruForm) {
			NormalArray[BufferedVerts] = NormalArray[BufferedVerts - 2];
		}
		...
		if (UseTruForm) {
			FGLNormal &destNormalArray = NormalArray[BufferedVerts];
			destNormalArray.x = P->Normal.X;
			destNormalArray.y = P->Normal.Y;
			destNormalArray.z = P->Normal.Z;
		}
	...
	if (UseTruForm) {
		FGLNormal *pNormal = &NormalArray[FirstVert];
		for (i = 0; i < NumPts; i++) {
			FixNormalIfNecessary(*(pNormal + i));
		}
	}
...
UBOOL UOpenGLRenderDevice::SetRes(INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen) {
	...
	if (DebugBit(DEBUG_BIT_BASIC)) {
		#define UTGLR_DEBUG_SHOW_PARAM(name) DbgPrintInitParam(TEXT(#name), name);

		UTGLR_DEBUG_SHOW_PARAM(LODBias);
		UTGLR_DEBUG_SHOW_PARAM(GammaOffset);
		UTGLR_DEBUG_SHOW_PARAM(GammaOffsetRed);
		UTGLR_DEBUG_SHOW_PARAM(GammaOffsetGreen);
		UTGLR_DEBUG_SHOW_PARAM(GammaOffsetBlue);
		UTGLR_DEBUG_SHOW_PARAM(Brightness);
		UTGLR_DEBUG_SHOW_PARAM(GammaCorrectScreenshots);
		UTGLR_DEBUG_SHOW_PARAM(OneXBlending);
		UTGLR_DEBUG_SHOW_PARAM(MaxLogUOverV);
		UTGLR_DEBUG_SHOW_PARAM(MaxLogVOverU);
		UTGLR_DEBUG_SHOW_PARAM(MinLogTextureSize);
		UTGLR_DEBUG_SHOW_PARAM(MaxLogTextureSize);
		UTGLR_DEBUG_SHOW_PARAM(MaxAnisotropy);
		UTGLR_DEBUG_SHOW_PARAM(TMUnits);
		UTGLR_DEBUG_SHOW_PARAM(MaxTMUnits);
		UTGLR_DEBUG_SHOW_PARAM(RefreshRate);
		UTGLR_DEBUG_SHOW_PARAM(UseZTrick);
		UTGLR_DEBUG_SHOW_PARAM(UseBGRATextures);
		UTGLR_DEBUG_SHOW_PARAM(UseMultiTexture);
		UTGLR_DEBUG_SHOW_PARAM(UsePalette);
		UTGLR_DEBUG_SHOW_PARAM(UsePrecache);
		UTGLR_DEBUG_SHOW_PARAM(ShareLists);
		UTGLR_DEBUG_SHOW_PARAM(AlwaysMipmap);
		UTGLR_DEBUG_SHOW_PARAM(UseTrilinear);
		UTGLR_DEBUG_SHOW_PARAM(AutoGenerateMipmaps);
//		UTGLR_DEBUG_SHOW_PARAM(UseVertexSpecular);
		UTGLR_DEBUG_SHOW_PARAM(UseAlphaPalette);
		UTGLR_DEBUG_SHOW_PARAM(UseS3TC);
		UTGLR_DEBUG_SHOW_PARAM(Use16BitTextures);
		UTGLR_DEBUG_SHOW_PARAM(UseTNT);
		UTGLR_DEBUG_SHOW_PARAM(UseCVA);
		UTGLR_DEBUG_SHOW_PARAM(NoMaskedS3TC);
		UTGLR_DEBUG_SHOW_PARAM(NoFiltering);
		UTGLR_DEBUG_SHOW_PARAM(DetailMax);
		UTGLR_DEBUG_SHOW_PARAM(UseDetailAlpha);
		UTGLR_DEBUG_SHOW_PARAM(DetailClipping);
		UTGLR_DEBUG_SHOW_PARAM(ColorizeDetailTextures);
		UTGLR_DEBUG_SHOW_PARAM(SinglePassFog);
		UTGLR_DEBUG_SHOW_PARAM(SinglePassDetail);
//		UTGLR_DEBUG_SHOW_PARAM(BufferActorTris);
		UTGLR_DEBUG_SHOW_PARAM(BufferClippedActorTris);
		UTGLR_DEBUG_SHOW_PARAM(BufferTileQuads);
		UTGLR_DEBUG_SHOW_PARAM(UseSSE);
		UTGLR_DEBUG_SHOW_PARAM(UseSSE2);
		UTGLR_DEBUG_SHOW_PARAM(UseTexIdPool);
		UTGLR_DEBUG_SHOW_PARAM(UseTexPool);
		UTGLR_DEBUG_SHOW_PARAM(CacheStaticMaps);
		UTGLR_DEBUG_SHOW_PARAM(DynamicTexIdRecycleLevel);
		UTGLR_DEBUG_SHOW_PARAM(TexDXT1ToDXT3);
		UTGLR_DEBUG_SHOW_PARAM(UseMultiDrawArrays);
		UTGLR_DEBUG_SHOW_PARAM(UseVertexProgram);
		UTGLR_DEBUG_SHOW_PARAM(SwapInterval);
		UTGLR_DEBUG_SHOW_PARAM(FrameRateLimit);
#ifdef UTGLR_DX_BUILD
		UTGLR_DEBUG_SHOW_PARAM(SceneNodeHack);
#endif
		UTGLR_DEBUG_SHOW_PARAM(SmoothMaskedTextures)
		UTGLR_DEBUG_SHOW_PARAM(MaskedTextureHack);
		UTGLR_DEBUG_SHOW_PARAM(RequestHighResolutionZ);
		UTGLR_DEBUG_SHOW_PARAM(UseAA);
		UTGLR_DEBUG_SHOW_PARAM(NumAASamples);
		UTGLR_DEBUG_SHOW_PARAM(AAFilterHint);
		UTGLR_DEBUG_SHOW_PARAM(ZRangeHack);
		UTGLR_DEBUG_SHOW_PARAM(UseTruForm);
		UTGLR_DEBUG_SHOW_PARAM(TruFormTessellation);
		UTGLR_DEBUG_SHOW_PARAM(TruFormMinVertices);

		#undef UTGLR_DEBUG_SHOW_PARAM
	}
	...
	if (!SUPPORTS_GL_ATI_pn_triangles) UseTruForm = 0;
	...
	PL_UseSSE = UseSSE;
	PL_UseSSE2 = UseSSE2;
	PL_UseTruForm = UseTruForm;
	PL_TruFormTessellation = TruFormTessellation;
...
void UOpenGLRenderDevice::InitARBPixelFormat(INT NewColorBytes, InitARBPixelFormatRet_t *pRet) {
	...
	initParams.RequestHighResolutionZ = RequestHighResolutionZ;
	...
	case WM_CREATE:
		{
			...
			INT DesiredDepthBits   = ((NewColorBytes <= 2) && !pInitParams->RequestHighResolutionZ) ? 16 : 24;
...
void UOpenGLRenderDevice::SetBasicPixelFormat(INT NewColorBytes) {
	...
	INT DesiredDepthBits   = ((NewColorBytes <= 2) && !RequestHighResolutionZ) ? 16 : 24; // NewColorBytes<=2 ? 16 : 32;
	PIXELFORMATDESCRIPTOR pfd =
	{
		sizeof(PIXELFORMATDESCRIPTOR),
		1,
...
bool UOpenGLRenderDevice::SetAAPixelFormat(INT NewColorBytes) {
	...
	iAttributes[7] = ((NewColorBytes <= 2) && !RequestHighResolutionZ) ? 16 : 24;
...
UBOOL UOpenGLRenderDevice::Init(UViewport* InViewport, INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen) {
	...
	debugf(TEXT("Initializing ROpenGLDrv..."));
...
#ifdef UTGLR_DEBUG_SHOW_CALL_COUNTS
{
	...
	if (NoMaskedS3TC != PL_NoMaskedS3TC) {
		PL_NoMaskedS3TC = NoMaskedS3TC;
		flushTextures = true;
	}
	...
	if (UseTruForm != PL_UseTruForm) {
		PL_UseTruForm = UseTruForm;
		if (UseTruForm) {
			InitTruFormSafe();
		}
	}
	if (TruFormTessellation != PL_TruFormTessellation) {
		PL_TruFormTessellation = TruFormTessellation;
		if (UseTruForm) {
			SetTruFormTessellation();
		}
	}
	...
	if (UseTruForm) {
		m_pBuffer3BasicVertsProc = Buffer3Verts;
		m_pBuffer3ColoredVertsProc = Buffer3Verts;
		m_pBuffer3FoggedVertsProc = Buffer3Verts;
	}
	...
	if (UseVertexProgram) {
		...
		m_pRenderPassesNoCheckSetup_DualTextureAndDetailTextureProc = &UOpenGLRenderDevice::RenderPassesNoCheckSetup_DualTextureAndDetailTexture_VP;
	}
	else {
		m_pRenderPassesNoCheckSetupProc = &UOpenGLRenderDevice::RenderPassesNoCheckSetup;
		m_pRenderPassesNoCheckSetup_DualTextureAndDetailTextureProc = &UOpenGLRenderDevice::RenderPassesNoCheckSetup_DualTextureAndDetailTexture;
#ifdef UTGLR_INCLUDE_SSE_CODE
		if (UseSSE) {
			m_pRenderPassesNoCheckSetupProc = &UOpenGLRenderDevice::RenderPassesNoCheckSetup_SSE;
			m_pRenderPassesNoCheckSetup_DualTextureAndDetailTextureProc = &UOpenGLRenderDevice::RenderPassesNoCheckSetup_DualTextureAndDetailTexture_SSE;
		}
#endif //UTGLR_INCLUDE_SSE_CODE
	}
	...
	if (ColorizeDetailTextures) {
		m_detailTextureColor3f[0] = 0.25f;
		m_detailTextureColor3f[1] = 0.5f;
		m_detailTextureColor3f[2] = 0.25f;
		m_detailTextureColor4ub = 0x00408040;
	}
	else {
		m_detailTextureColor3f[0] = 0.5f;
		m_detailTextureColor3f[1] = 0.5f;
		m_detailTextureColor3f[2] = 0.5f;
...
#ifdef UTGLR_DEBUG_SHOW_CALL_COUNTS
{
	...
	AddRenderPass(Surface.Texture, PolyFlags, 0);

	if (Surface.MacroTexture) {
		AddRenderPass(Surface.MacroTexture, PF_Modulated, -0.5);
	}

	if (Surface.LightMap) {
		AddRenderPass(Surface.LightMap, PF_Modulated, -0.5);
	}

	UOpenGLRenderDevice::DrawDistanceFog(Frame); //renders distance fog
	...
	if (Surface.FogMap) {
		...
		AddRenderPass(Surface.FogMap, PF_Highlighted, -0.5);
	}
...
//DISTANCE FOG STUFF
/*
since fog color var doesn't work to me of unknown reasons i've used lightcolor var instead:
LightBrightness -> R in RGB color
LightHue -> G in RGB color
LightSaturation -> B in RGB color
*/
void UOpenGLRenderDevice::DrawDistanceFog(FSceneNode* Frame) {
#ifdef UTGLR_DEBUG_SHOW_CALL_COUNTS
{
	static int si;
	dout << L"utglr: DrawDistanceFog = " << si++ << std::endl;
}
#endif
	guard(UOpenGLRenderDevice::DrawDistanceFog);

	ALevelInfo* levelInfo;  // initialise it to zero in the appropriate place (at point of definition or in the constructor)
	ULevel* currentLevel;   // and another one
	if (Frame && (!levelInfo || Frame->Level != currentLevel)) // update if we change levels
	{
		currentLevel = Frame->Level;
		if (currentLevel)
		{
			levelInfo = currentLevel->GetLevelInfo();
		}
	}

	float FDI=0; //all var's must be set to 0 at the begining
	GLfloat ColorR=0.0f;
	GLfloat ColorG=0.0f;
	GLfloat ColorB=0.0f;
	GLfloat AlphaB=0.0f;
	APlayerPawn *p;
	p = (APlayerPawn *)Frame->Viewport->Actor; //player pawn that owns teh viewport
	if(p)
	{
		if(p->Region.Zone)
		{
			ColorR=p->Region.Zone->LightBrightness;
			ColorG=p->Region.Zone->LightHue;
			ColorB=p->Region.Zone->LightSaturation;
			FDI=p->Region.Zone->FogDistance;
		}
		else //if we haven't find zone info we will use global settings stored in LevelInfo
		{
			ColorR=levelInfo->LightBrightness;
			ColorG=levelInfo->LightHue;
			ColorB=levelInfo->LightSaturation;
			FDI=levelInfo->FogDistance;
		}
	}
	//if fog color is black we will use gray color.
	if(ColorR == 0 && ColorB == 0 && ColorB == 0)
	{
		ColorR=0.5f;
		ColorG=0.5f;
		ColorB=0.5f;
	}
	//user can force distance fog for levels that dosn't have distance fog support
	if((ForceDistanceFog) || ((FDI < 128) && SpawnIfNoDistanceFog))
	{
		ColorR=FogColorR;
		ColorG=FogColorG;
		ColorB=FogColorB;
	...
	//there is no sese in rendering distance fog less then 128 units.
	if(FDI >= 128) 
	{
		glEnable(GL_FOG); //we have to enable fog
		{
				GLfloat fogColor[4] = {ColorR / 255.0f, ColorG / 255.0f, ColorB / 255.0f, AlphaB / 255.0f};
				glClearColor(fogColor[0], fogColor[1], fogColor[2], fogColor[3]); //clears color
				glFogi(GL_FOG_MODE, GL_LINEAR);       //we use linear fog
				glFogfv(GL_FOG_COLOR, fogColor);      //sets fog color
				glFogf(GL_FOG_DENSITY, 0.35f);        //sets fog density
				glHint(GL_FOG_HINT, GL_NICEST);       //doesn't work in some cases (on ATI cards I think)
				glClearColor(fogColor[0], fogColor[1], fogColor[2], fogColor[3]); //clears color
				glFogf(GL_FOG_START, 0.0f);           //fog start (player position)
				glFogf(GL_FOG_END, FDI);              //fog end (FDI->fog distance)
		}
	}
	else //if fog distance is less or equal 0 we have to disable fog!!
	{
		glDisable(GL_FOG); 
	}
	unguard;
...
#ifdef UTGLR_DEBUG_SHOW_CALL_COUNTS
{
	...
	if (UseTruForm) {
		m_requestedColorFlags |= CF_TRUFORM;
	}
	...
	for (INT i = 0; i < NumPts; i++) {
		...
		if (UseTruForm) {
			FGLNormal &destNormal = NormalArray[Index];
			destNormal.x = P->Normal.X;
			destNormal.y = P->Normal.Y;
			destNormal.z = P->Normal.Z;
			FixNormalIfNecessary(destNormal);
		}
...
#ifdef UTGLR_DEBUG_SHOW_CALL_COUNTS
{
	...
	glBegin(GL_LINES);
	glVertex3f(m_RFX2 * P1.Z * (P1.X - Frame->FX2), m_RFY2 * P1.Z * (P1.Y - Frame->FY2), P1.Z);
	glVertex3f(m_RFX2 * P2.Z * (P2.X - Frame->FX2), m_RFY2 * P2.Z * (P2.Y - Frame->FY2), P2.Z);
	glEnd();
...
#ifdef UTGLR_DEBUG_SHOW_CALL_COUNTS
{
	...
	glBegin(GL_TRIANGLE_FAN);
	FLOAT X1Pos = m_RFX2 * Z * (X1 - Frame->FX2);
	FLOAT Y1Pos = m_RFY2 * Z * (Y1 - Frame->FY2);
	FLOAT X2Pos = m_RFX2 * Z * (X2 - Frame->FX2);
	FLOAT Y2Pos = m_RFY2 * Z * (Y2 - Frame->FY2);
...
void UOpenGLRenderDevice::InitTruFormSafe(void) {
	//Set fixed and variable TruForm state parameters
...
void UOpenGLRenderDevice::SetTruFormTessellation(void) {
	GLint tessellationLevel;
	GLint maxTessellationLevel = 0;

	tessellationLevel = TruFormTessellation;
	glGetIntegerv(GL_MAX_PN_TRIANGLES_TESSELATION_LEVEL_ATI, &maxTessellationLevel);
	if (tessellationLevel < 0) tessellationLevel = 0;
	if (tessellationLevel > maxTessellationLevel) tessellationLevel = maxTessellationLevel;

	glPNTrianglesiATI(GL_PN_TRIANGLES_TESSELATION_LEVEL_ATI, tessellationLevel);
...
}
{
	...
	if ((Info.Format == TEXF_DXT1) && SupportsTC) {
		if (TexDXT1ToDXT3 && (!(PolyFlags & PF_Masked))) {
			...
			//texSourceFormat not used for compressed textures
			pBind->texInternalFormat = GL_COMPRESSED_RGB_S3TC_DXT1_EXT;
			if ((PolyFlags & PF_Masked) && !NoMaskedS3TC) {
...
void UOpenGLRenderDevice::SetTexEnvNoCheck(INT Multi, DWORD texEnvFlags) {
	guardSlow(UOpenGLRenderDevice::SetTexEnv);

	//Update current tex env flags early as there are no subsequent dependencies
	m_curTexEnvFlags[Multi] = texEnvFlags;
	
	if (texEnvFlags & PF_Modulated) {
		if ((texEnvFlags & PF_FlatShaded) || (Multi != 0) && !OneXBlending) {
...
void UOpenGLRenderDevice::SetDefaultColorStateNoCheck(void) {
	//Check for TruForm
	if (m_currentColorFlags & CF_TRUFORM) {
		glDisable(GL_PN_TRIANGLES_ATI);
		glDisableClientState(GL_NORMAL_ARRAY);
	}

	//Check for fog mode
	if (m_currentColorFlags & CF_FOG_MODE) {
		glDisable(GL_COLOR_SUM_EXT);
		glDisableClientState(GL_SECONDARY_COLOR_ARRAY_EXT);

		//Reset color array pointer to default when not in fog mode
		glColorPointer(4, GL_UNSIGNED_BYTE, sizeof(FGLSingleColor), &SingleColorArray[0].color);
	}
...
void UOpenGLRenderDevice::SetColorStateNoCheck(void) {
	...
	//Check for TruForm change
	if (changedFlags & CF_TRUFORM) {
		if (m_requestedColorFlags & CF_TRUFORM) {
	...
	//Check for fog mode change
	if (changedFlags & CF_FOG_MODE) {
		if (m_requestedColorFlags & CF_FOG_MODE) {
			glEnable(GL_COLOR_SUM_EXT);
			glEnableClientState(GL_SECONDARY_COLOR_ARRAY_EXT);

			//Set up color array pointer for fog mode
...
void UOpenGLRenderDevice::AllocateVertexProgramNamesSafe(void) {
	...
	//Allocate complex surface names
...
bool UOpenGLRenderDevice::InitializeVertexPrograms(void) {
	...
	if (TMUnits >= 2) {
		...
		initOk &= LoadVertexProgram(m_vpComplexSurface[1], g_vpComplexSurfaceDoubleTexture,
...
void UOpenGLRenderDevice::RenderPassesExec_DualTextureAndDetailTexture(FTextureInfo &DetailTextureInfo) {
...
#ifdef UTGLR_INCLUDE_SSE_CODE
//Must be called with (m_rpPassCount > 0)
void UOpenGLRenderDevice::RenderPassesNoCheckSetup_SSE(void) {
	INT i;
	INT t;
...
//Must be called with (m_rpPassCount > 0)
void UOpenGLRenderDevice::RenderPassesNoCheckSetup_DualTextureAndDetailTexture(FTextureInfo &DetailTextureInfo) {
	INT i;
	INT t;
	FLOAT NearZ  = 380.0f;
	...
	glColor3fv(m_detailTextureColor3f);
	...
	DisableSubsequentClientTextures(4);
...
#ifdef UTGLR_INCLUDE_SSE_CODE
//Must be called with (m_rpPassCount > 0)
void UOpenGLRenderDevice::RenderPassesNoCheckSetup_DualTextureAndDetailTexture_SSE(FTextureInfo &DetailTextureInfo) {
	INT i;
	INT t;
	FLOAT NearZ  = 380.0f;
	FLOAT RNearZ = 1.0f / NearZ;
...
//Must be called with (m_rpPassCount > 0)
void UOpenGLRenderDevice::RenderPassesNoCheckSetup_DualTextureAndDetailTexture_VP(FTextureInfo &DetailTextureInfo) {
	INT i;
...
void UOpenGLRenderDevice::DrawDetailTexture(FTextureInfo &DetailTextureInfo, INT BaseClipIndex, bool clipDetailTexture) {
	...
	do {
		...
		for (DWORD PolyNum = 0; PolyNum < m_csPolyCount; PolyNum++, pNumPts++, pDetailTextureIsNear++) {
			...
			//Otherwise, no clipping required, or clipping required, but DetailClipping not enable
...
void UOpenGLRenderDevice::EndBufferingNoCheck(void) {
	//No need to set default projection state here as EndBufferingNoCheck sets its own projection state
	//No need to set default color state here as EndBufferingNoCheck sets its own color state
	SetDefaultVertexProgramState();
	SetDefaultTextureState();
	...
	//Update TruForm color state flag
	if (UseTruForm) {
		if (BufferedVerts >= TruFormMinVertices) {
			m_requestedColorFlags |= CF_TRUFORM;
		}
		else {
			m_requestedColorFlags &= ~CF_TRUFORM;
...
void UOpenGLRenderDevice::EndTileBufferingNoCheck(void) {
	SetDefaultProjectionState();
	//No need to set default color state here as EndTileBufferingNoCheck sets its own color state
	SetDefaultVertexProgramState();
	SetDefaultTextureState();
