...
static const TCHAR *g_pSection = TEXT("RD3D8Drv.D3D8RenderDevice");
...
#define CPP_PROPERTY_LOCAL(name) name, CPP_PROPERTY(name)
	...
	SC_AddBoolConfigParam(6,  TEXT("UseMultiTexture"), CPP_PROPERTY_LOCAL(UseMultiTexture), 1);
	SC_AddBoolConfigParam(5,  TEXT("UsePalette"), CPP_PROPERTY_LOCAL(UsePalette), 0);
	...
	SC_AddBoolConfigParam(1,  TEXT("UseS3TC"), CPP_PROPERTY_LOCAL(UseS3TC), 0);
...
#ifdef UTGLR_DX_BUILD
	SC_AddBoolConfigParam(7,  TEXT("SceneNodeHack"), CPP_PROPERTY_LOCAL(SceneNodeHack), 0);
#endif
...
//		UTGLR_DEBUG_SHOW_PARAM(BufferActorTris);
		...
		UTGLR_DEBUG_SHOW_PARAM(UseTripleBuffering);
		UTGLR_DEBUG_SHOW_PARAM(UsePureDevice);
		UTGLR_DEBUG_SHOW_PARAM(UseSoftwareVertexProcessing);
...
UBOOL UD3D8RenderDevice::Init(UViewport* InViewport, INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen) {
	...
	if (!hModuleD3d8) {
		...
		if (!hModuleD3d8) {
			debugf(NAME_Init, TEXT("Failed to load %s"), appFromAnsi(g_d3d8DllName));
			return 0;
		}
		pDirect3DCreate8 = (LPDIRECT3DCREATE8)GetProcAddress(hModuleD3d8, "Direct3DCreate8");
		if (!pDirect3DCreate8) {
			debugf(NAME_Init, TEXT("Failed to load function from %s"), appFromAnsi(g_d3d8DllName));
			return 0;
		}
	}
...
// DISTANCE FOG!!!
//=============================================================================
	UD3D8RenderDevice::DrawDistanceFog(Frame);
//=============================================================================
// DISTANCE FOG!!!
//=============================================================================
	if (Surface.FogMap) {
...
//DISTANCE FOG STUFF
void UD3D8RenderDevice::DrawDistanceFog(FSceneNode* Frame)
{
	guard(UD3D8RenderDevice::DrawDistanceFog);
	...
	FLOAT ColorB=0.0f;
	FLOAT AlphaB=0.0f;
	FColor FogColor = FColor(0.5f,0.5f,0.5f);  // base fog color
	APlayerPawn *p;
	...
	if(p)
	{
		if(p->Region.Zone)
		{
			FogColor.R = p->Region.Zone->FogColor.G;
			FogColor.G = p->Region.Zone->FogColor.B;
			FogColor.B = p->Region.Zone->FogColor.A;
			FogColor.A = p->Region.Zone->FogColor.R;
			ColorR=FogColor.R;
			ColorG=FogColor.G;
			ColorB=FogColor.B;
		...
		{
			FogColor.R = levelInfo->FogColor.G;
			FogColor.G = levelInfo->FogColor.B;
			FogColor.B = levelInfo->FogColor.A;
			FogColor.A = levelInfo->FogColor.R;
			ColorR=FogColor.R;
			ColorG=FogColor.G;
			ColorB=FogColor.B;
	...
	if(FDI > 128)            //...if FogDistance is greater then 0 we will set fog color
	{
		//Enable fog
		D3DCOLOR fogColor = D3DCOLOR_COLORVALUE(ColorR/255.0f, ColorG/255.0f, ColorB/255.0f, 1.0f);
		m_d3dDevice->SetRenderState(D3DRS_FOGENABLE, TRUE);
		m_d3dDevice->SetRenderState(D3DRS_FOGCOLOR, fogColor);
		m_d3dDevice->SetRenderState(D3DRS_FOGTABLEMODE, D3DFOG_LINEAR);
		m_d3dDevice->SetRenderState(D3DRS_FOGSTART, *(DWORD*)&FogStart);
		m_d3dDevice->SetRenderState(D3DRS_FOGEND, *(DWORD*)&FDI);
	}
	unguard;
}
#ifdef UTGLR_RUNE_BUILD
...
}
{
	...
	if ((Info.Format == TEXF_DXT1) && SupportsTC) {
		...
		if (PolyFlags & PF_Memorized) {
			pBind->texFormat = (PolyFlags & PF_Masked) ? D3DFMT_A1R5G5B5 : D3DFMT_R5G6B5;
		}
	}
	else {
		pBind->texType = TEX_TYPE_NORMAL;
		if (texFlags & TEX_FLAG_NO_CLAMP) {
			pBind->pConvertBGRA7777 = &UD3D8RenderDevice::ConvertBGRA7777_BGRA8888_NoClamp;
		}
		else {
			pBind->pConvertBGRA7777 = &UD3D8RenderDevice::ConvertBGRA7777_BGRA8888;
		}
		pBind->texFormat = D3DFMT_A8R8G8B8;
	}
...
void UD3D8RenderDevice::SetTexEnvNoCheck(DWORD texUnit, DWORD texEnvFlags) {
	...
	if (texEnvFlags & PF_Modulated) {
		...
		m_d3dDevice->SetTextureStageState(texUnit, D3DTSS_ALPHAOP, texOp);
