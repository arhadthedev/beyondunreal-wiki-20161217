...
#define MAX_TEX_SIZE 256 // 512 works but may slow down framerate because of texture memory thrashing. EDIT: Bigger means better :P
...
{
    DECLARE_CLASS(UD3DRenderDevice,URenderDevice,CLASS_Config,RD3DDrv)
...
//distance fog stuff
	...
	FLOAT FogDistance;
	FLOAT FDI;
	BITFIELD ForceDistanceFog;
	BITFIELD SpawnIfNoDistanceFog;
    ...
    void StaticConstructor()
	{
		...
		new(GetClass(),TEXT("FogColorR"),          RF_Public)UIntProperty  ( CPP_PROPERTY(FogColorR          ), TEXT("Options"), CPF_Config );
		new(GetClass(),TEXT("FogColorG"),          RF_Public)UIntProperty  ( CPP_PROPERTY(FogColorG          ), TEXT("Options"), CPF_Config );
		new(GetClass(),TEXT("FogColorB"),          RF_Public)UIntProperty  ( CPP_PROPERTY(FogColorB          ), TEXT("Options"), CPF_Config );
		new(GetClass(),TEXT("FogDistance"),          RF_Public)UIntProperty  ( CPP_PROPERTY(FogDistance          ), TEXT("Options"), CPF_Config );
		new(GetClass(),TEXT("ForceDistanceFog"),        RF_Public)UBoolProperty ( CPP_PROPERTY(ForceDistanceFog        ), TEXT("Options"), CPF_Config );
		new(GetClass(),TEXT("SpawnIfNoDistanceFog"),        RF_Public)UBoolProperty ( CPP_PROPERTY(SpawnIfNoDistanceFog        ), TEXT("Options"), CPF_Config );
...
//DISTANCE FOG STUFF
void DrawDistanceFog(FSceneNode* Frame)
{
	guard(UD3DRenderDevice::DrawDistanceFog);
	...
	{
		//Enable fog
		D3DCOLOR fogColor = D3DRGB(ColorR/255.0f, ColorG/255.0f, ColorB/255.0f, 1.0f);
		...
		Direct3DDevice7->SetRenderState( D3DRENDERSTATE_FOGTABLEEND, *(DWORD*)&FDI);
	}
	unguard;
}
	...
	void SetBlending( DWORD PolyFlags )
	{
		...
		{
			PolyFlags &= ~PF_Masked;
			D3DCOLOR BlackFogColor = D3DRGB(0.0f, 0.0f, 0.0f);
			Direct3DDevice7->SetRenderState( D3DRENDERSTATE_FOGCOLOR, BlackFogColor);
		}
		else if (PolyFlags & PF_Modulated) {
			PolyFlags &= ~PF_Masked;
			D3DCOLOR GrayFogColor = D3DRGB(0.5f, 0.5f, 0.5f);
			Direct3DDevice7->SetRenderState( D3DRENDERSTATE_FOGCOLOR, GrayFogColor);
		}
