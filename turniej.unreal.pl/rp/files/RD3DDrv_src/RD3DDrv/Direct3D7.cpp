...
{
    DECLARE_CLASS(UD3DRenderDevice,URenderDevice,CLASS_Config,TCOD3DDrv)
...
//=============================================================================
		FLOAT FogStart=0.f;
		FLOAT FogDistance;
  	        APlayerPawn *p; //player pawn that owns teh viewport :)
		ATCODistanceFogZone *ZI; //zone info (TCODistanceFogZone) in which player is
		p = (APlayerPawn *)Frame->Viewport->Actor; //we read playerpawn...
		ZI = (ATCODistanceFogZone *)p->Region.Zone; //...and zone info (TCODistanceFogZone)
		FLOAT ColorR=0.0f;
		FLOAT ColorG=0.0f;
		FLOAT ColorB=0.0f;
		FLOAT AlphaB=0.0f;
		if(ZI) //if (TCODistanceFogZone) exists
		{
			FogDistance=ZI->FogDistance; //we have to set Fog Distance...
			if(FogDistance > 0) //...if FogDistance is greater then 0 we will set fog color
			{
				ColorR=ZI->FogColorR;
				ColorG=ZI->FogColorG;
				ColorB=ZI->FogColorB;
		...
		D3DCOLOR fogColor = D3DRGB(ColorR/255, ColorG/255, ColorB/255);
		if(FogDistance > 0)
		{
